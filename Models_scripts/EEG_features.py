...
# local
import tools as st
import plotting_eeg_feat, utiles_eeg_feat
...
def eeg(signal=None, sampling_rate=1000., labels=None, path=None, show=True):
    ...
    # plot
    if show:
        plotting_eeg_feat.plot_eeg(ts=ts,
                          raw=signal,
                          filtered=filtered,
                          labels=labels,
                          features_ts=ts_feat,
    ...
    return utiles_eeg_feat.ReturnTuple(args, names)
...
def car_reference(signal=None):
    ...
    return utiles_eeg_feat.ReturnTuple((out,), ('signal',))
