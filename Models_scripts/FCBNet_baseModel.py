...
import Models_scripts.stopCriteria_FBCNet as stopCriteria_FBCNet
import Models_scripts.samplers_FBCNet as samplers_FBCNet
...
class baseModel():
    ...
    def _trainOE(
        ...
        stopCondition = stopCriteria_FBCNet.composeStopCriteria(**stopCondi)
            ...
            if doStop:
                ...
                if continueAfterEarlystop:
                    if not earlyStopReached:
                        ...
                        stopCondition = stopCriteria_FBCNet.composeStopCriteria(**modifiedStop)
                    else:
                        bestNet = copy.deepcopy(self.net.state_dict())
    ...
    def trainOneEpoch(self, trainData, lossFn, optimizer, sampler = None):
        ...
        for d in dataLoader:
            with torch.enable_grad():
                ...
                # forward pass:
                output = self.net(d[0].unsqueeze(1).to(self.device))

                # calculate loss
                loss = lossFn(output, d[1].type(torch.LongTensor).to(self.device))
                loss = loss/d[0].shape[0]
                # backward pass:
                loss.backward()
                optimizer.step()
    ...
    def predict(self, data, sampler = None, lossFn = None):
        ...
        with torch.no_grad():
            # iterate over all the data
            for d in dataLoader:
                preds = self.net(d[0].unsqueeze(1).to(self.device))
                totalCount += d[0].shape[0]

                if lossFn is not None:
                    # calculate loss
                    loss += lossFn(preds, d[1].type(torch.LongTensor).to(self.device)).data
    ...
    def _findSampler(self, givenString):
        ...
        elif givenString in samplers_FBCNet.__dict__.keys():
            out = samplers_FBCNet.__dict__[givenString]
        else:
