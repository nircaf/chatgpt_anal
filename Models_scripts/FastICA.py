...
def FastICA(X, vectors, eps):
	...
	iter = 0
	prev_dist = np.inf
	while( np.abs(prev_dist - np.abs(np.dot(w1.T,w2))) < eps):
		if iter > 0:
			prev_dist = np.abs(np.dot(w1.T,w2))
		w1 = w2
		...
		w2 = w2/np.linalg.norm(w2)
		if iter % 100 == 0:
			print(f'The error is {np.abs(np.dot(w1.T,w2))} with delta of: {np.abs(prev_dist - np.abs(np.dot(w1.T,w2)))} @ iter: {iter}')
