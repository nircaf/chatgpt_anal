...
class ReassignedDataset(data.Dataset):
    ...
    def __init__(self, image_indexes, pseudolabels, dataset, transform=None, true_target=None, args=None, epoch=0, labels_name=None):
        if args is None:
            self.imgs = self.make_dataset(image_indexes, pseudolabels, dataset, true_target)
        else:
            if labels_name is None:
                self.imgs = self.make_dataset(image_indexes, pseudolabels, dataset, true_target, if_vis_psaudo=args.if_vis_psaudo, epoch=epoch)
            else:
                self.imgs = self.make_dataset(image_indexes, pseudolabels, dataset, true_target,
                                              if_vis_psaudo=args.if_vis_psaudo, epoch=epoch, labels_name=labels_name, args=args)
        self.transform = transform
        self.args = args

    def make_dataset(self, image_indexes, pseudolabels, dataset, true_target, if_vis_psaudo=False, epoch=0, labels_name=None, args=None):
        label_to_idx = {label: idx for idx, label in enumerate(set(pseudolabels))}
        images = []
        for j, idx in enumerate(image_indexes):
            item = dataset[idx, :, :]
            pseudolabel = label_to_idx[pseudolabels[j]]
            true_targets = true_target[j]
            images.append((item, pseudolabel, true_targets))
        if if_vis_psaudo:
            np_pseudolabel = np.asarray(pseudolabels)
            labels = np.unique(true_target)
            normalized_centerd_psaudo_labels = len(labels) * (np_pseudolabel - min(np_pseudolabel)) / (max(np_pseudolabel) - min(np_pseudolabel))
            colors = cm.rainbow(np.linspace(0, 1, len(labels)))
            fig, ax = plt.subplots()
            for lbl, c in zip(labels, colors):
                logic_vec = (np.asarray(true_target) == lbl)
                x = np.linspace(lbl, lbl + 1, sum(logic_vec))
                cur_psaudo_labels = normalized_centerd_psaudo_labels[logic_vec]
                if labels_name is None:
                    ax.scatter(x, cur_psaudo_labels, c=c)
                else:
                    ax.scatter(x, cur_psaudo_labels, c=c, label=f"{labels_name[lbl]}")
            num_of_unique_psaudo_labels = len(np.unique(np_pseudolabel))
            # produce a legend with the unique colors from the scatter
            if labels_name is not None:
                ax.legend(loc="upper left", title="Classes")

            plt.xlabel('True labels region')
            plt.ylabel('Pseudo labels label region')
            plt.suptitle(f'Clustering performance in epoch #{epoch}, with {num_of_unique_psaudo_labels} groups')


            if args is not None:
                plt.savefig(os.path.join(args.save_true_vs_pseudo_figures_path, f'performance in epoch_{epoch}.png'))
    ...
    def __getitem__(self, index):
        ...
        """
        item, pseudolabel, true_target = self.imgs[index]
        img = item
...
def cluster_assign(images_lists, item=None, True_target=None, args=None, epoch=0, labels_name=None):
    ...
    Returns:
        ReassignedDataset(torch.utils.data.Dataset): a dataset with clusters as
                                                     labels
    """
    np_True_target = True_target.numpy()
    ...
    for cluster, images in enumerate(images_lists):
        ...
        True_target_labels.extend(np_True_target[images])
    ...
    if args is None:
        return ReassignedDataset(image_indexes, pseudolabels, item, None, True_target_labels)
    else:
        return ReassignedDataset(image_indexes, pseudolabels, item, None, True_target_labels, args=args, epoch=epoch, labels_name=labels_name)
