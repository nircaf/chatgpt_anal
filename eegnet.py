    ...
    for root, dirs, files in os.walk(start_dir):
        ...
        if any(file.endswith(".edf") for file in files) and any(file.endswith(".txt") for file in files):
            ...
            for filename in filenames:
                ...
                raw_recording,labels = eeg_dft_array_w_fft(eeg_recording,seizures)
                xdata=raw_recording
                label=labels
                label.astype(int)
                samplenum=label.shape[0]
            #   there are 11 subjects in the dataset. Each sample is 3-seconds data from 30 channels with sampling rate of 128Hz.
                channelnum=eeg_recording.info['nchan']
            #   define the learning rate, batch size and epoches
                lr=1e-3
                # batch_size = closest_to_n(xdata.shape[1])
                n_epoch =11

        # select the subject index here
        #       form the training data
        #         x_train = xdata.reshape(batch_size,1,channelnum, xdata.shape[1]//batch_size)[0:int(xdata.shape[0]*0.8),:,:,:]
        #         y_train = label[0:int(label.shape[0]*0.8)]
        # #       form the testing data
        #         x_test = xdata.reshape(batch_size,1,channelnum, xdata.shape[1]//batch_size)[int(xdata.shape[0]*0.8):,:,:,:]
        #         y_test = label[int(label.shape[0]*0.8):]
                # torch train test split
                xdata = xdata.reshape(xdata.shape[0],1,xdata.shape[1],xdata.shape[2])
                data_set = torch.utils.data.TensorDataset(torch.from_numpy(xdata), torch.from_numpy(label))
                train_size = int(0.8 * len(data_set))
                test_size = len(data_set) - train_size
                train,test = random_split(data_set, [train_size, test_size])
                batch_size = 50
                train_loader = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=True)

        # select the deep learning model to be used
                my_net = InterpretableCNN(sampleChannel=channelnum).double().cuda()
                # my_net = EEGNet(channelnum=channelnum).double().cuda()
                # import compactcnn
                # my_net = compactcnn.CompactCNN(classes=2 ,channels=channelnum,kernelLength=sf//6,sampleLength=sf).double().cuda()
        ...
        # train the classifier
                for epoch in range(n_epoch):
                    for j, data in enumerate(train_loader, 0):
                        ...
                        optimizer.step()
                    # print loss
                    print("Epoch {} - Loss: {:.4f}".format(epoch, err.detach().item()))

                my_net.eval()
                results = []
                with torch.no_grad():
                    for j, x_test in enumerate(test, 0):
                        x_test, y_test = data
                        answer = my_net(x_test.cuda())
...
def closest_to_n(x,n=100):
  # Start with 50
  closest = n

  # If 50 is divisible by x, return 50
  if x % closest == 0:
    return closest

  # If 50 is not divisible by x, increment or decrement by 1 until a number is found that is divisible by x
  while closest % x != 0:
    closest += 1 if closest > x else -1
    closest_to_n(x)
  # Return the closest number that is divisible by x
  return closest
...
def eeg_net_run(eeg_recording,X_train,Y_train,X_validate, Y_validate,X_test,Y_test):
    channelnum=eeg_recording.info['nchan']
    samples = len(X_train)
    #   define the learning rate, batch size and epoches
    lr=1e-3
    ...
    checkpointer = ModelCheckpoint(filepath='/home/tzag/psycho/chatgpt_anal/checkpoint.h5', verbose=1,
                                save_best_only=True)
    ...
    X_train      = X_train.reshape(X_train.shape[0], channelnum, samples)
    X_test       = X_test.reshape(X_test.shape[0], channelnum, samples)
