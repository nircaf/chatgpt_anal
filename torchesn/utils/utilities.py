...
def washout_tensor(tensor, washout, seq_lengths, bidirectional=False, batch_first=False):
    ...
    for b in range(tensor.size(1)):
        if washout[b] > 0:
            tmp = tensor[int(washout[b]):int(seq_lengths[b]), b].clone()
            tensor[:int(seq_lengths[b] - washout[b]), b] = tmp
            tensor[int(seq_lengths[b] - washout[b]):, b] = 0
    ...
    return tensor[:int(max_len)], seq_lengths
